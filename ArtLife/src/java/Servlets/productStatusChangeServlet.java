        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                if (!request.getParameter("deactiveBtn").equals(null) & request.getParameter("deactiveBtn").equals("Deactive")) {
                    ...
                    productCriteria.add(Restrictions.eq("id", Integer.parseInt(request.getParameter("productId").toString())));

                    if (productCriteria.uniqueResult() != null) {
                        Pojos.Products productUpdate = (Pojos.Products) productCriteria.uniqueResult();

                        productUpdate.setStatus(0);
                        productUpdate.setUuid(Integer.parseInt(request.getSession().getAttribute("uid").toString()));
                        productUpdate.setUpdatedAt(new Date());

                        hiberSession.update(productUpdate);
